<script>
	/**
	 * Chart 4: Normalized Throughput vs TTFT (Smooth Curve, Linear Scales)
	 * Shows relationship between latency and normalized performance
	 */
	import {
		createMargins,
		getInnerDimensions,
	...
	let {
		...
		width = 400,
		height = 300,
		title = 'Normalized Throughput vs TTFT'
	} = $props();
	...
	<svg {width} {height} class="font-instrument-sans h-auto w-full overflow-visible">
		...
		<g transform="translate({margin.left}, {margin.top})">
			...
			<g clip-path="url(#{clipId})">
				...
				{#each data as point, i (i)}
					<circle
						...
						role="img"
						aria-label="Data point at {point.x}ms TTFT, {point.y} normalized throughput"
			...
			<g class="y-axis">
				...
				>
					Normalized Throughput (tokens/sec/chip)
				</text>
			</g>
	...
	{#if tooltipData}
		...
		>
			<div class="space-y-0.5 text-xs text-slate-600 dark:text-slate-400">
				<div><strong>TTFT:</strong> {tooltipData.x}ms</div>
				<div>
					<strong>Normalized Throughput:</strong>
					{formatYAxis(tooltipData.y)} tok/s/chip
				</div>
				{#if tooltipData.original}
					<div><strong>Concurrent Users:</strong> {tooltipData.original.concurrentUsers}</div>
					<div>
						<strong>System Throughput:</strong>
